...
SoundCloud got rid of its API's ability to download songs, so the program listens for responses containing "mp3", downloads these splices of a song, and pieces them together to combat this.

NOTE: When downloading these MP3 snippets, some of them lead and end with short bits of silence. To combat this and make the final export smoother, the program tries to detect these bits of silence on each splice and get rid of it, but this could result in sonic artifacts if there is intended silence in the song. To turn this feature off, there is a toggleable boolean variable at the top of the script called trim. 
...
---
Spotify Subplaylist Maker is a Python script that takes a playlist link or a user's liked songs and creates a filtered clone based on audio feature factors decided by the user (e.g., keeping songs that have high energy, high danceability, moderate loudness, and are in C major).
